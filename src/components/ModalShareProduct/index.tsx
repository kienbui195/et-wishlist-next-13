...
import { FacebookIcon, PinterestIcon, LinkedinIcon, TwitterIcon, CloseButton, DefaultThumbnail2 } from "@/utils/svgExport";
  ...
  return (
    <div>
      <div className="flex w-full justify-end pb-4 pr-5 pt-5" ref={closeSharePrd.current}>
        <Image src={CloseButton} alt="" className="my-px cursor-pointer" onClick={() => onClose()} />
      </div>
      ...
      <div className="break-normal text-center text-base font-normal leading-5 tracking-wide text-[--text-gray] mx-auto max-w-[382px]">
        Share {prdDetail?.attributes.name} with friends or on social media to boost its exposure and support its creators.
      </div>
      <div className="relative mb-8 mt-6 flex overflow-hidden rounded-lg border border-gray-1500 bg-white mx-[40px] max-w-full shadow-productShareInfo cardShare">
        <div className="group/items rounded-10 mr-5 flex h-[72px] w-[72px] shrink-0 items-center justify-center overflow-hidden rounded-10 md:mr-[30px] md:h-[160px] md:w-[160px]">
          <video
            src={`${process.env.NEXT_PUBLIC_BE_URL}${prdDetail?.attributes.hoverVideo?.data?.attributes.url}`}
        ...
        <div className="mr-2 mt-[22px] grow md:mt-0 md:flex md:max-w-[390px] md:flex-col md:justify-center md:pb-[9px] md:pt-2.5">
          <div
            ...
            onClick={() => {
              ...
              router.push(`/products/${prdDetail?.attributes?.slug}`);
            }}
          >
            <h2 title={prdDetail?.attributes.name} className="product-title line-clamp-1 text-ellipsis font-[RobotoBold]">
          ...
          <div className=" md:flex-wrap hidden md:flex mt-2.5">
            {Array.from(prdDetail?.attributes?.tags.data || []).map((_i: any, idx: number) => {
              return idx < 3 ? (
                <Link
                  key={idx}
                  href={`/tag/${_i.attributes.slug}?_pt=${prdDetail?.attributes.productType}`}
      ...
      <div className="flex h-[74px] items-center justify-center shadow-productShareNetwork">
        ...
        <div
          ...
          onClick={() => {
            ...
            navigator.clipboard.writeText(`${window.location.href}products/${prdDetail?.attributes?.slug}`);
            handleCopyLink();
          }}
        >
          Copy Link
