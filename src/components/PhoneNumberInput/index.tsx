...
const PhoneNumberInput: FC<IPhoneNumberInputProps> = ({
  ...
  const [selected, setSelected] = useState(valueSelected ? valueSelected.name : "");
  ...
  return (
    <div className={`flex flex-col items-stretch space-y-3 box-border ${className}`}>
      ...
      <div className={`flex flex-col space-y-0.5 `}>
        <div className={`flex justify-between items-center border-[1px] border-[--gray-line] rounded-[4px] ${error ? "!border-[--state-error]" : ""}`}>
          <div className="relative">
            <div className={`outline-none px-[10px] py-[9px] bg-white cursor-pointer h-[44px] flex items-center`} onClick={() => setOpen(!open)} ref={inputRef}>
              <div className="flex justify-between items-center space-x-3 w-full">
                <div className="font-[400] text-[15px] leading-[15px] text-[--gray-text] select-none flex-1">{selected || placeholder}</div>
                <Image
            ...
            <div
              ...
              className={`w-fit bg-white rounded-[4px] border-[1px] border-[--gray-line] flex-col items-stretch max-h-[242px] overflow-x-hidden absolute z-30 left-0 top-[50px] shadow-[_-12px_17px_24px_0px_#4B4B7140] ${
                open ? `transform h-[${list.length > 0 ? "fit-content" : "44px"}] duration-300 flex` : "transform duration-300 h-0 hidden"
              } 
         ${list.length * 44 >= 242 ? "overflow-y-auto" : "overflow-y-hidden"} scrollBar
         `}
            >
          ...
          <div className={`flex flex-col items-stretch space-y-3 box-border flex-1`}>
            <div className="flex flex-col items-stretch space-y-0.5">
              <div className={`${error ? "!border-[--state-error]" : ""} h-[44px] p-[0_8px] flex items-center space-x-2 whitespace-nowrap`}>
        ...
        {error && labelError && labelError !== "" && <div className="text-[10px] leading-[10px] text-[--state-error]">{labelError}</div>}
        {hint && hint !== "" && <div className="text-[10px] text-[--gray2] leading-[10px]">{hint}</div>}
      </div>
    </div>
  );
};
