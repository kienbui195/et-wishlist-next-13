...
import { ITopBannerImage } from "@/data/wl-types";
...
const CustomCarousel: FC<ICarouselProps> = ({
  ...
  const handleNext = () => {
    setSelectedGlrImg(selectedGlrImg === images.length - 1 ? 0 : selectedGlrImg + 1);
  };

  const handlePrev = () => {
    setSelectedGlrImg(selectedGlrImg === 0 ? images.length - 1 : selectedGlrImg - 1);
  };
  ...
  const renderButtonNext = (isRight = false) => {
    return (
      <div
        className={`${!isRight ? "rotate-180" : ""} cursor-pointer group/icon p-3 rounded-full hover:bg-[--carousel-hover] hidden`}
  ...
  return (
    ...
    >
          ...
          {images.map((_i, idx) => {
            return (
              <div
                ...
                style={{
                  transform: `translateX(${(idx - selectedGlrImg) * 100}%) scale(1)`,
                  ...(height ? { height: `${height}px` } : {}),
                }}
              >
                <div className={`flex ${imageHeight} ${imageWidth} items-center justify-center `}>
                  <Link href={_i.link_on_click ? `${_i.link_on_click}` : "/#"} target="_blank">
                    <Image
                      src={`${process.env.NEXT_PUBLIC_BE_URL}${_i.image.data?.attributes.url}`}
                      alt=""
                      className={` ${imageHeight} ${imageWidth} object-contain cursor-pointer w-full h-auto`}
                      width={0}
                      height={0}
                      sizes="100vw"
                    />
      ...
      <div className={`absolute w-fit left-0 h-full top-0 ${images.length > 1 ? "flex" : "hidden"} flex-col justify-center`}>{renderButtonNext()}</div>
      <div className={`absolute w-fit right-0 h-full top-0 ${images.length > 1 ? "flex" : "hidden"} flex-col justify-center`}>{renderButtonNext(true)}</div>
      {images.length > 1 && (
        <div className="flex justify-center items-center  absolute bottom-0  w-full">
          <div className="p-1 bg-transparent hover:bg-[--carousel-hover] flex items-center rounded-full">
            {images.map((_, index) => (
              <div
                key={index}
                className={`h-2 w-2 mx-1 rounded-full cursor-pointer ${index === selectedGlrImg ? "bg-[--gray-line]" : "bg-[--gray-text]"}`}
