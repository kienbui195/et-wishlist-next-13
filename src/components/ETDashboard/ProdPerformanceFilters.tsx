...
export type TProdPerformanceFilters = "votes" | "page_views" | "orders" | "sales";
...
const ProdPerformanceFilters: FC<IProdPerformanceFiltersProps> = ({ onChange }) => {
  ...
  return (
    <div className="flex rounded-md border-[1.4px] border-gray-2550 bg-white shadow-dashboardButtons">
      {PROD_PERFORMANCE_FILTERS.map((item: IDropdownItem, idx: number) => {
        return <Fragment key={idx}>{renderRow(item, selected === item.id)}</Fragment>;
      })}
    </div>
  );
};
