...
import { validateFeed, validateMedia, validateMoreInfo, validateOurStory, validatePrd, validateSProduct } from "./helper";
...
const CreateProduct: React.FC<CreateProductProps> = ({ isCreate }) => {
  ...
  const [loadingSubmitPageBtn, setLoadingSubmitPageBtn] = useState<boolean>(false);
        ...
        .then((res) => {
          ...
          const data: CreateProductFormState = {
            ...
            prodDtl: {
              ...
              productPageHeadline: attributes.prodDtl?.productPageHeadline || "",
            ...
            moreInfo: {
              ...
              returnExchangePolicy: attributes.moreInfo?.returnExchangePolicy || "",
              whatIncluded: attributes.moreInfo?.whatIncluded || "",
              whatIncludedTitle: "What's Included",
            },
            launchDate: attributes.launchDate ? moment(attributes.launchDate).format("YYYY-MM-DD") : undefined,
            tags,
            category: {
            ...
            storyClip: {
              ...
              url: attributes.storyClip?.data?.attributes.clip?.data?.attributes.url,
            },
          };
          setForm({ ...form, ...data });
        })
    ...
    if (
      ...
      apis
        ...
        .then((res) => {
          ...
          router.push("/brand-products/edit/" + data.id);
      ...
      showAlert("warning", "Please fill all required fields or limit characters");
  ...
  useEffect(() => {
    ...
    document.title = `ET Wishlist | Brand Products - ${isCreate ? "Create" : "Edit"}`;
  ...
  const renderDashBoardPanel = () => {
    switch (prdPannel) {
        ...
        return (
          <PanelConnection
            ...
            callback={(value: SpProduct) => {
              ...
              const shopVariantCurrency = value?.priceRangeV2?.minVariantPrice?.currencyCode;
              setForm((preState) => ({
                ...preState,
      ...
      case "feed":
        return <PanelFeed data={form} onSetState={setForm} isCreate={isCreate} errors={errorsFeed} />;
      case "prdPage":
        return <PanelPrdPage data={form} onSetState={setForm} errors={errorsPrd} />;
      case "images":
        return <PanelImages data={form} onSetState={setForm} errors={errorsMedia} />;
      case "ourStory":
        return <PanelOurStory data={form} onSetState={setForm} errors={errorsOurStory} />;
      case "moreInfo":
        return <PanelMoreInfo data={form} onSetState={setForm} errors={errorsMoreInfo} />;
      case "faq":
      ...
      <main className="w-full px-10 py-7">
        <div>
          <div className="flex w-full items-start justify-between">
            <div>
              <span className="cursor-pointer text-10 font-bold leading-none tracking-wide text-gray-1150" onClick={() => router.push("/brand-products")}>
                BACK TO MY PRODUCTS
              </span>
              <h2 className="mt-5 text-base font-semibold leading-none tracking-tight text-black">{isCreate ? "Create" : "Edit"} Product Page</h2>
              <h3 className="font-sm mt-[9px] leading-none text-gray-1150">Complete the sections below to publish your product page.</h3>
            </div>
            <div className="text-grey-2650 flex h-7 items-center whitespace-nowrap rounded bg-white px-2 text-13 font-medium shadow-brandButton">
              {form.pageApproved ? "Page Approved" : form.pageSubmitted ? "Page Submitted" : "Page Not Submitted"}
              <div
                className="ml-2 block h-[7px] w-[7px] rounded-full"
                style={{
                  backgroundColor: form.pageApproved ? "#00C71E" : form.pageSubmitted ? "#0078FF" : "#8E9DAE",
                }}
              ></div>
            </div>
          </div>
          <div className="ml-[571px] mt-[30px] flex justify-end">
            {!isCreate && (
              <Fragment>
                {/* TODO: use Link react-router-dom to router.push to product detail */}
                <Link href={`/products/${form.slug}`} target="_blank">
                  <DashboardButton label="Preview Page" type="white" />
                </Link>
                <DashboardButton label="Submit Page" onClick={loadingSubmitPageBtn ? () => {} : handleSubmitPage} type="white" />
              </Fragment>
            )}
            <DashboardButton label="Save Changes" onClick={loadingSaveBtn ? () => {} : isCreate ? handleCreateProduct : handleEditProduct} />
          </div>
              ...
              <div className="el-tabs__header is-top">
                <div className="el-tabs__nav-wrap is-top">
                  <div className="el-tabs__nav-scroll">
                    <div className="el-tabs__nav is-top" role="tablist" style={{ transform: "translateX(0px);" }}>
                      <div
                        className={`el-tabs__item is-top ${prdPannel === "connection" ? "is-active" : ""}`}
                        id="tab-Connections"
                      ...
                      <div
                        className={`el-tabs__item is-top ${prdPannel === "feed" ? "is-active" : ""}`}
                        id="tab-Feed"
                      ...
                      <div
                        className={`el-tabs__item is-top ${prdPannel === "prdPage" ? "is-active" : ""}`}
                        id="tab-Product Page"
                      ...
                      <div
                        className={`el-tabs__item is-top ${prdPannel === "images" ? "is-active" : ""}`}
                        id="tab-Images"
                      ...
                      <div
                        className={`el-tabs__item is-top ${prdPannel === "ourStory" ? "is-active" : ""}`}
                        id="tab-Our Story"
                      ...
                      <div
                        className={`el-tabs__item is-top ${prdPannel === "moreInfo" ? "is-active" : ""}`}
                        id="tab-More Info"
                      ...
                      <div
                        className={`el-tabs__item is-top ${prdPannel === "faq" ? "is-active" : ""}`}
                        id="tab-FAQs"
