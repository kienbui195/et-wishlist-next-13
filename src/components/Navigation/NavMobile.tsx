...
const NavMobile: React.FC<NavMobileProps> = ({
      ...
      <div key={index}>
        ...
        >
          <div
            ...
            product-title-22 box-border ${
              location.pathname.replace('/', '') === item.slug ||
          ...
          {item.isNew && (
            <div className={'px-2 bg-red-400 rounded-full text-white text-sm'}>
              New
            </div>
          )}
        </Link>
  ...
  return (
    <div className={`overflow-y-auto w-full h-screen bg-white`}>
      <div
        className="p-[4px_16px] border-b-[1px] border-[--gray-line] h-[56px] flex items-center justify-between bg-[#F6F6F6]"
      >
        ...
        {!token && (
          <CustomButton
            label="Sign Up"
            className="!h-[40px] bg-[--gray-text] text-white rounded-[4px]"
            onClick={() => dispatch(setOpenModalLogin(true))}
          />
        )}
      </div>
      {userInfo.id  && <React.Fragment>
      ...
      <div className="p-[10px_24px] h-[52px] flex items-center">
        {renderSearchForm()}
      </div>
      <div className="flex flex-col space-y-6 m-[4px_32px] ">
        ...
        {!token ? (
          ...
          <Fragment>
            {NAV_OPTION_LOGIN.map((item: INavigation, idx: number) =>
