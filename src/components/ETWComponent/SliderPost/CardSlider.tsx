...
import { ArrowRight, PrdAwdMonth, PrdAwdWeek, PrdAwdYear, VotedIcon2 } from "@/utils/svgExport";
...
const CardSlider: FC<CardSliderProps> = ({ heading, subHeading, className = "", posts, perView = 4, prefixHref = "" }) => {
                  ...
                  {posts.map((_i, indx) => {
                    return (
                      <motion.li
                        ...
                        onClick={() => router.push(`/${prefixHref}/${_i.href}`)}
                      >
                        <div>
                          <div className="relative pb-[10px]">
                            <Image src={_i.image} className="w-[200px] h-[200px] object-cover" alt="" width={0} height={0} sizes="100vw" />
                            <div className="absolute bottom-[18px] left-[6px] flex items-center justify-start space-x-1">
                              ...
                              {_i.award.map((award: number, idx: number) => {
                                ...
                                switch (award) {
                                  case 1:
                                    image = <Image src={PrdAwdWeek} alt="" key={idx} />;
                                    break;
                                  case 2:
                                    image = <Image src={PrdAwdMonth} alt="" key={idx} />;
                                    break;
                                  case 3:
                                    image = <Image src={PrdAwdYear} alt="" key={idx} />;
                                    break;
                                  default:
                                    break;
                                }
                          ...
                          <div className="R-home-content line-clamp-2 text-ellipsis cardDetailTitle text-[--gray-text]">{_i.title}</div>
                        </div>
                      </motion.li>
                    );
                  })}
