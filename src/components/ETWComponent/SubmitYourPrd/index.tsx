...
function SubmitYourPrd() {
  ...
  const userInfo = localStorage.getItem("ETWL") && JSON.parse(localStorage.getItem("ETWL") || "");
  const queryShop = useSearchParams().get("shop");
  ...
  return installShopifyApp ? (
    <div className="w-full h-full md:et-container et-container-sm ">
      ...
      {step === 4 && <Step4 step={step} nextStep={() => {}} backStep={() => setStep(3)} />}
