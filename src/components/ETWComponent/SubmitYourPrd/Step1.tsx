...
import { MailIcon, PersonIcon } from "@/utils/svgExport";
...
const Step1: FC<Step1Props> = ({ nextStep }) => {
  ...
  let [, codePart = "", phoneNumberPart = ""] = (userInfo && userInfo.phone && userInfo.phone.match(REGEX.SPLIT_AREA_CODE_VS_PHONE_NUMBER)) || ["", "+1", ""];
  let indexCode = PHONE_NUMBER_CODE.findIndex((item) => item.id === codePart);
  let nameCodePart: string = codePart ? (indexCode !== -1 ? PHONE_NUMBER_CODE[indexCode].name : "United States") : "United States";
  ...
  const handleData = (e: any) => {
    ...
    if (listError.firstName === "" && listError.lastName === "" && listError.role === "" && listError.reasonKnowET === "" && listError.phone === "") {
      setErrors({
        firstName: "",
        lastName: "",
        phone: "",
  ...
  return (
    <main className="w-full flex-1">
      <div className="mt-8">
        ...
        <div className="mt-[9px] text-base font-semibold leading-none text-green-1450">1 of 3 {/*1 of 4{' '}*/}</div>
        <h1 className="mt-[35px] text-xl font-semibold leading-none text-black">Tell us about yourself.</h1>
      ...
      <div className="flex">
        <div className="w-[555px]">
          <div className="text-lg font-semibold leading-tight text-gray-1050"> Your E-Mail Address </div>
          <form className="el-form el-form--default el-form--label-top mb-0 mt-[25px]">
            <FormInput label="E-Mail Address" required value={userInfo.email} disabled startIcon={MailIcon} />
          </form>
          <div className="mt-3 text-xs font-normal leading-normal text-gray-2050 hidden">
            {" "}
            In order to change e-mail please <span className="cursor-pointer font-semibold text-black underline underline-offset-2"> reset account </span>
          </div>
        </div>
        ...
        <form className="el-form el-form--default el-form--label-top">
          <div className="text-lg font-semibold leading-tight text-gray-1050"> Your Contact Details </div>
