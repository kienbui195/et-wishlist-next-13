...
const MultiSelect: FC<IMultiSelectProps> = ({
  ...
  const renderDropdownRow = (
    ...
    return (
      ...
      >
        {name}
        {selected.some((item) => item.id === id) && <img src={CheckedIcon} alt=''/>}
      </div>
    )
  }
  ...
  const renderSelected = () => {
    return (
      <div
        tabIndex={-1}
        className="w-full whitespace-normal flex items-center flex-wrap cursor-pointer space-x-[11px]"
      >
        {selected.map((item, idx) => (
          <div key={idx} className="box-border">
            <div className="text-[--el-color-info] px-[9px] h-[24px] space-x-[6px] flex items-center justify-center bg-[#F4F4F5] rounded-[4px]">
              {item.name}
              <div className="w-[15px] h-[15px] p-[1px] rounded-[50%] cursor-pointer hover:bg-[#91949A] hover:text-white"
      ...
      <div className="relative " id={uid + ''}>
        ...
        >
          <div className="flex justify-between items-center space-x-3 w-full">
            ...
            <div
              className={`font-[400] text-[15px] leading-[15px] text-[--gray-text] select-none flex-1`}
            >
              {!textHidden ? (
                selected.length > 0 ? (
                  renderSelected()
        ...
        } overflow-x-hidden absolute z-30 left-0 top-[${
            58 + (error ? 12 : 0) + (hint !== '' ? 12 : 0)
          }px] 
