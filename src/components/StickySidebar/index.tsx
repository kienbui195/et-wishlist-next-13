    ...
    const handleScroll = () => {
      ...
      if (window.innerHeight < sidebarHeight) {
        const stickyTopSidebarHomepage =
          sidebarHomepage && sidebarHomepage?.getBoundingClientRect().top <= 100 ? sidebarHomepage?.getBoundingClientRect().top : TOPBANNER_STICKY;

        const currentScrollPos = window.pageYOffset;
        if (currentScrollPos > prePosWindow.current) {
          if (Math.abs(stickyTopSidebarHomepage - window.scrollY + prePosScroll.current - 10) <= sidebarHeight - window.innerHeight) {
            setSetstickyBanner(stickyTopSidebarHomepage - window.scrollY + prePosScroll.current + 10);
          } else {
            setSetstickyBanner(-sidebarHeight + window.innerHeight - 10);
          }
        } else {
          prePosScroll.current = window.scrollY;
          setSetstickyBanner(stickyTopSidebarHomepage < TOPBANNER_STICKY ? stickyTopSidebarHomepage + 10 : TOPBANNER_STICKY);
        }
      ...
      const caculateViewVideo = Array.from(listVideoEl)
        .map((_i: any, idx) => {
          ...
          const inView =
            boundingClientRect.top - -(1 - displayPercentage) * (boundingClientRect.bottom - boundingClientRect.top) > 0 &&
            boundingClientRect.top + thresholdHeight <= windowHeight;
          return (
            inView && {
              video: _i,
      ...
      if (videoNearest) {
        ...
        pauseAllVideos();
        const isPlaying =
          videoNearest.currentTime > 0 && !videoNearest.paused && !videoNearest.ended && videoNearest.readyState > videoNearest.HAVE_CURRENT_DATA;
        if (!isPlaying && videoNearest.readyState >= videoNearest.HAVE_CURRENT_DATA) {
