...
const Curation = ({ params }: { params: { slug: string } }) => {
  ...
  return (
    <div className="w-full px-4 lg:px-0 mainContent">
        ...
        <div className="flex space-x-4 min-[480px]:flex-row flex-col ">
          <div className="flex space-x-4 items-center">
            ...
            <Link
              ...
              href={`/?category=${curation.attributes.category.data?.attributes.slug}`}
            >
            ...
            <CustomButton
              ...
              label={
                <React.Fragment>
                  ...
                  <div className="w-6 h-6">
                    {curation.isVoted ? <Image src={VotedIcon} alt="" /> : <IconButtonVoteAnimation />}
                  </div>
              ...
              onClick={
                !loadingVote
                  ? !curation.isVoted
                    ? handleVote
                    : () => window.open(curation.attributes.ctaButton[0]?.link, "_blank")
                  : () => {}
              }
            />
          </div>
          ...
          <div className="flex flex-col flex-1 space-y-6 mt-8">
            <div className="flex items-start flex-col gap-2 leading-10">
              ...
              {curation.attributes.inStock > 0 && (
                <div className="bg-yellow-300 text-green-500 px-3 py-1 text-[16px] leading-none rounded">in stock</div>
              )}
            </div>
            {curation.attributes.ctaButton[0] && (
              <CTAButton ctaButton={curation.attributes.ctaButton[0]} className="max-w-full sm:max-w-[350px]" />
            )}
            <div>{curation.attributes.merchant}</div>
            <div className="text-link-16 italic">
              as of{" "}
              {moment(curation.attributes.crawTime || curation.attributes.publishedAt).format("MMMM DD, YYYY h:mm a")}
            </div>
          </div>
        </div>
      ...
      {curation.attributes.ctaButton[2] && curation.attributes.faqs.length > 0 && (
        <CTAButton ctaButton={curation.attributes.ctaButton[2]} className="mt-8" />
      )}
