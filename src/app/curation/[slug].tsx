...
// import CurationPrdGalleryModal from './Component/CurationPrdGalleryModal'
...
const Curation = () => {
  ...
  const handleGetPageData = () => {
    apis
      ...
      .then((res) => {
        ...
        setCurationGallery(
          curationData.attributes.images.data ? prod.attributes.images.data : []
        )
      })
      .catch((err) => {
        console.log(err)
  ...
  const handleShowPrice = () => {
    const currencySymbol = getCurrencyCode(
      curation.attributes.currency ? curation.attributes.currency : 'USD'
    )
          ...
          <span>
            {formatDataNumber(
              calculatePriceApplyDiscount(
                ...
                curation.attributes.discountType
              ),
              2
            )}
          </span>
  ...
  useEffect(() => {
    const calculateSizeVideoWrapper = () => {
      ...
      let imgHeight = windowHeight - 326

      if (windowHeight <= 768) {
        imgHeight = DEFAULT_MEDIA_WIDTH * 0.75
      }
      if (windowHeight > 768 && windowHeight <= 900) {
        imgHeight = DEFAULT_MEDIA_WIDTH * 0.9
      }
      if (windowHeight > 900 && windowHeight <= 1060) {
        imgHeight = DEFAULT_MEDIA_WIDTH
      }
      if (windowHeight < 1230 && windowHeight > 1060) {
        imgHeight = DEFAULT_MEDIA_WIDTH * 1.1
      }
      if (windowHeight >= 1230) {
        imgHeight = DEFAULT_MEDIA_WIDTH * 1.2
      }

      setCardHeight(innerWidth < 631 ? innerWidth : imgHeight)
    }
    ...
    <div className="w-full px-4 lg:px-0 mainContent">
      <div className="intro-hook flex flex-col space-y-4 mt-8">
        <Title
          ...
          title={curation.attributes.headline}
        />
        <div className="flex space-x-4 min-[480px]:flex-row flex-col ">
          <div className="flex space-x-4 items-center">
              ...
              <img
                ...
                src={
                  companyInfo.logo?.data
                    ? `${process.env.REACT_APP_URL_BE}${companyInfo.logo.data?.attributes.url}`
                    : DefaultAvatar
                }
                loading='lazy'
              />
            ...
            <Link
              ...
              to={`/?category=${curation.attributes.category.data?.attributes.slug}`}
            >
          ...
          <div className="h-14 grow items-center justify-end self-end flex space-x-3">
            ...
            >
              <img
                className="h-7 w-7"
                src={
                  curation.isBookmarked ? CurationBookmarked : CurationBookmark
                }
                alt=""
              />
            </div>
              ...
              label={
                <React.Fragment>
                  <div className="md:flex hidden whitespace-nowrap">
                    {curation.isVoted ? 'Buy Now' : 'Upvote'}
                  </div>
                  <div className="w-6 h-6">
                    {curation.isVoted ? (
                  ...
                  <div>
                    {curation.attributes.votes
                      ? formatDataNumber(curation.attributes.votes, 0)
                      : 0}
                  </div>
                </React.Fragment>
              }
              onClick={
                !loadingVote
                  ? !curation.isVoted
                    ...
                    : () =>
                        window.open(
                          curation.attributes.ctaButton[0]?.link,
                          '_blank'
                        )
                  : () => {}
              }
          ...
          {curationGallery.length > 0 ? (
            curationGallery.length < 3 ? (
              <Card1
                ...
                onClick={(url) => {
                  const index = curationGallery.findIndex(
                    (item) => item.attributes.url === url
                  )
                  setInitIndex(index)
                }}
        ...
        />
        <InnerHtml
          className="long-intro space-y-2"
          text={curation.attributes.longIntro}
        />
      </div>
      <div className="short-desc flex flex-col space-y-6 mt-8">
        <Title
          className="capitalize font-extrabold"
          title={curation.attributes.name}
        />
        <div className="flex flex-col sm:flex-row sm:space-x-8 space-y-8">
          <div className="w-full h-auto flex-1">
            <img
              ...
              src={`${process.env.REACT_APP_URL_BE}${
                curation.attributes.images.data
                  ? curation.attributes.images.data[0]?.attributes.url
                  : DefaultThumbnail2
              }`}
              loading='lazy'
            />
          ...
          <div className="flex flex-col flex-1 space-y-6 mt-8">
            <div className="flex items-start flex-col gap-2 leading-10">
              {handleShowPrice()}
              {curation.attributes.inStock > 0 && (
                <div className="bg-yellow-300 text-green-500 px-3 py-1 text-[16px] leading-none rounded">
                  in stock
                </div>
              )}
            </div>
            {curation.attributes.ctaButton[0] && (
              <CTAButton
                ctaButton={curation.attributes.ctaButton[0]}
            ...
            <div className="text-link-16 italic">
              as of{' '}
              {moment(
                curation.attributes.crawTime || curation.attributes.publishedAt
              ).format('MMMM DD, YYYY h:mm a')}
            </div>
          </div>
        </div>
        <InnerHtml
          className="short-desc space-y-2"
          text={curation.attributes.shortDesc}
        />
        <div className="feature flex flex-col space-y-4">
          {curation.attributes.featuresTitle &&
            curation.attributes.featuresTitle.trim().length > 0 && (
              <Title
                className="bg-yellow-500 capitalize px-5 py-2 rounded-lg"
                title={curation.attributes.featuresTitle}
              />
            )}
          {curation.attributes.features?.split('\n').map((_i, idx) => (
            <div key={idx} className="flex space-x-4 items-center">
              <img className="w-6 h-6" src={CheckIconYellow} alt="" />
      ...
      <div className="pros-cons mt-8">
        <div className="flex flex-col sm:flex-row space-y-8 sm:space-y-0 space-x-0 sm:space-x-8">
          {curation.attributes.pros &&
            curation.attributes.pros.trim().length > 0 && (
              <div className="w-full sm:w-1/2 p-5 flex flex-col space-y-8 shadow-lg rounded-lg">
                <Title
                  title={
                    <div className="flex space-x-4 items-center">
                      <img alt="" className="w-12 h-12" src={ProsIcon} />
                      <span>Pros</span>
                    </div>
                  }
                />
                <div className="flex flex-col space-y-4">
                  {curation.attributes.pros?.split('\n').map((_i, idx) => (
                    <div key={idx} className="flex space-x-4 items-center">
                      <img alt="" src={CheckIcon} className="w-6 h-6" />
                      <InnerHtml className="text-[16px] space-y-2" text={_i} />
                    </div>
                  ))}
                </div>
          ...
          {curation.attributes.cons &&
            curation.attributes.cons.trim().length > 0 && (
              <div className="w-full sm:w-1/2 p-5 flex flex-col space-y-8 shadow-lg rounded-lg">
                <Title
                  title={
                    <div className="flex space-x-4 items-center">
                      <img alt="" className="w-12 h-12" src={ConsIcon} />
                      <span>Cons</span>
                    </div>
                  }
                />
                <div className="flex flex-col space-y-4">
                  {curation.attributes.cons?.split('\n').map((_i, idx) => (
                    <div key={idx} className="flex space-x-4 items-center">
                      <img alt="" src={CloseIcon} className="w-6 h-6" />
                      <InnerHtml className="text-[16px] space-y-2" text={_i} />
                    </div>
                  ))}
                </div>
          ...
          <div className="mt-8">
            ...
            <YouTube
              ...
              videoId={getYoutubeId(
                curation.attributes.linkYoutube?.split('\n')[0]
              )}
      ...
      <div className="mt-8 flex flex-col space-y-4">
        {curation.attributes.wdwstp && (
          <Title title={curation.attributes.wdwstpTitle} />
        )}
        {curation.attributes.wdwstp?.split('\n').map((_i, idx) => {
          ...
          const randomIndex = Math.floor(
            Math.random() * (listIcon['current'].length - 1)
          )
          let chooseIcon = listIcon['current'][randomIndex]
          listIcon['current'].splice(randomIndex, 1)
          return (
            <div key={idx} className="flex space-x-4 items-center">
              <img alt="" src={chooseIcon} className="w-8 h-8" />
              <InnerHtml className="text-[16px]" text={_i} />
            </div>
          )
        })}
      </div>
      {curation.attributes.ctaButton[1] && (
        <CTAButton
          ctaButton={curation.attributes.ctaButton[1]}
          className="mt-8"
        />
      )}
      ...
      {curation.attributes.ctaButton[2] &&
        curation.attributes.faqs.length > 0 && (
          <CTAButton
            ctaButton={curation.attributes.ctaButton[2]}
            className="mt-8"
          />
        )}
      {sameCate.length > 0 ? (
      ...
      {!showShareModal && (
        <VoteLandMobile
          ...
          priceSale={calculatePriceApplyDiscount(
            curation.attributes.price,
            curation.attributes.discountValue,
            curation.attributes.discountType
          )}
